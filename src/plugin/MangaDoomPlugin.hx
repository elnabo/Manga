...
class MangaDoomPlugin extends Plugin
{
	...
	@override private function set_manga(value:String) {_manga = value; findLastChapterLocal(); imgRegex = new EReg("<img id.*?\"(http.*?)\".*?.*?wpm_nav_nxt = \"http://mangadoom.com/"+manga+"/([0-9]+)/","is"); return _manga;}
